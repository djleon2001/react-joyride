...
export default class Joyride extends React.Component {
  ...
  static propTypes = {
    ...
    showToursCallback: React.PropTypes.func,
  ...
  static defaultProps = {
    ...
    locale: {
      ...
      next: 'Next',
      skip: 'Skip',
      disable: 'Don\'t show me tours anymore.'
    },
  ...
  onClickTooltip(e) {
    ...
    if (el.className.indexOf('joyride-') === 0) {
      ...
      if (type === 'showTours') {                
        this.setState({
          showTours: !this.state.showTours
        });
        
        if (typeof props.showToursCallback === 'function' ) {
          props.showToursCallback(!this.state.showTours, props.steps[state.index]);
        }        
      } else {
        e.preventDefault();
        e.stopPropagation();
  ...
  toggleTooltip(show, index, action) {
    ...
    this.setState({
      play: props.steps[stepIndex] ? this.state.play : false,
      showTooltip: show,
      index: stepIndex,
      xPos: -1000,
      yPos: -1000
    }, () => {
      const lastIndex = action === 'back' ? stepIndex + 1 : stepIndex - 1;
